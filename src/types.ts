...
export interface SnowballYear {year:number;portfolio:number;contributions:number;capitalGrowth:number;grossDividends:number;netDividends:number;tax:number;reinvested:number;monthlyIncome:number;realPortfolio:number;unitPrice:number;shares:number;dividendPerUnit:number}
